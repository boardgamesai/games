...
func main() {
	...
	if len(flag.Args()) != 2 {
		log.Fatalf("Usage: go run main_fourinarow.go [-n <num games>] <player1> <player2>\n")
	}
	...
	for _, playerName := range flag.Args() {
		runnablePlayer, err := game.NewRunnablePlayer(config, "fourinarow", playerName)
		if err != nil {
	...
	if numGames == 1 {
		...
		outcomes := map[*fourinarow.Player]map[string]int{}
